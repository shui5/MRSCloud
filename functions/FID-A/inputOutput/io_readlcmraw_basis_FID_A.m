...
while ~feof(fid)
    ...
    if mod(vectorsize,2)==0
        ...
        %fids=fft(fftshift(specs,1),[],1); % changed fft to ifft, Jan 18, 2022, scnh
        fids=ifft(fftshift(specs,1),[],1);
    else
        %disp('Length of vector is odd.  Doing circshift by 1');
        %fids=fft(circshift(fftshift(specs,1),1),[],1); % changed fft to ifft, Jan 18, 2022, scnh
        fids=ifft(circshift(fftshift(specs,1),1),[],1);
    end
