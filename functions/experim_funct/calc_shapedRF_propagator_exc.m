...
function [Q_out] = calc_shapedRF_propagator_exc(H,RF,Tp,flipAngle,phase,dfdx,grad)
...
if ~RF_struct.isGM % if it is not gradient modulated pulse
    %if nargin>7 && grad~=0
    if nargin>6 && grad~=0 % scnh
        ...
        simType='g';
    else %if nargin==7 || (nargin>7 && grad==0)
...
else % if it is gradient modulated pulse
...
% if simType=='g' % Moved to the top - 09 June 2020
%     grad=grad*0.01;  %convert from G/cm to [T/m]
%     dfdx=dfdx/100;  %convert from cm to [m]
% end
...
for m=1:length(H)
    ...
    Rg=zeros(2^H(m).nspins,2^H(m).nspins);
    ...
    for y=1:H(m).nspins
        if simType=='g'
            ...
            %grad.*dfdx*gamma %scnh gradient offsets in Hz
            %H(m).Bfield*H(m).shifts(y)/1e6*gamma % scnh chemical offsets
            %in Hz
            %max(rfB1*gamma) % scnh rf amplitude in Hz
        elseif simType=='f'
    ...
    % for a slice selective excitation pulse, we will manually apply a
    % rephase gradient
    iota = grad.*dfdx*0.13*Tp*gamma*2*pi; %  Tp[sec],grad[T/m],dfdx[m],gamma[Hz/T],iota[rad]
    for y=1:H(m).nspins
        Rg=Rg+iota*H(m).Iz(:,:,y);
    end
    u=expm(1i*Rg);
    Q1=Q1*u; % uncertain whether to pre or post multiply. we are doing post multiply
    Q_out{m}=Q1;
end
