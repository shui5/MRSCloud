...
function [H,d] = sim_Hamiltonian_mgs(sys,Bfield)
...
for n=1:length(sys) %JN - Looping through the parts of the spin system:
    ...
    %Calculate the p-matrix, which shows the coherence order of each
    %element of the density matrix -- MGSaleh and RAEE June 11 2020
    p_order=[[0 -1];[1 0]];
    H(n).P_matrix=p_order;
    for ii=1:(H(n).nspins-1)
        H(n).P_matrix=[H(n).P_matrix H(n).P_matrix-1; H(n).P_matrix+1 H(n).P_matrix];
    end
    
    %Calculate F+ and F- -- MGSaleh and RAEE June 11 2020
    H(n).Fplus  = H(n).Fx + 1i*H(n).Fy;
    H(n).Fminus = H(n).Fx - 1i*H(n).Fy;
