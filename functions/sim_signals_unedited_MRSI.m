function [MRS_opt,outA,outB,outC,outD] = sim_signals_unedited_MRSI(MRS_opt)
...
for ii = 1:length(MRS_opt)
    ...
    switch (mega_or_hadam)
        case 'UnEdited_se_MRSI'
            taus     = TE;
%            delays(1) = taus - (MRS_opt(ii).refTp);
           
%fully slice-selective sequence
factor = (1-170/200); %0.13; %what fraction of the excitation pulse happens after the zero-phase point
           delays(2) = (taus - MRS_opt(ii).refTp)/2;
           delays(1) = (taus - MRS_opt(ii).refTp - 2*MRS_opt(ii).excTp*factor)/2; 
           %delays(1) = (taus - MRS_opt(ii).refTp)/2;% - 2*MRS_opt(ii).excTp*factor)/2; 
...
%            d=sim_apply_pfilter(d,MRS_opt(ii).H,+1);                   %Apply p_filter
% %fully slice-selective sequence
% %           d_A=sim_evolve(d,MRS_opt(ii).H,(delays(1))/2000);                     %Evolve by (delays(1)+delays(2))/2
 %           d_A=sim_evolve(d,MRS_opt(ii).H,(delays(1))/1000);                     %Evolve by (delays(1)+delays(2))/2
...
%           MRS_opt(ii).QoutONB    = calc_shapedRF_propagator_edit(MRS_opt(ii).H,MRS_opt(ii).editRFonB,MRS_opt(ii).editTp,MRS_opt(ii).edit_flipAngle,0);
    ...
    switch (mega_or_hadam)
        ...
        otherwise
            ...
            %if not PRESS
            if ~strcmp(MRS_opt(ii).seq, 'MEGA')&&~strcmp(MRS_opt(ii).seq, 'Edited_se_MRSI')
                [d_A,d_B,d_C,d_D]   = sim_megapress_shaped_ultrafast_edit1(MRS_opt(ii));
            elseif strcmp(MRS_opt(ii).seq, 'Edited_se_MRSI')
                    d       = sim_evolve(MRS_opt.d,MRS_opt.H,MRS_opt.delays(1)/1000);                      %Evolve by delays(2)
                ...
                d_A      = sim_evolve(d1,MRS_opt.H,MRS_opt.delays(2)/1000);                             %Evolve by delays(3)
                d_B      = sim_evolve(d2,MRS_opt.H,MRS_opt.delays(2)/1000);  
            else
...
%% fully slice-selective sequence
 parfor Y=1:length(MRS_opt(ii).y) %Use this if you do have the MATLAB parallel processing toolbox
    ...
    %[outA_temp{Y}]=sim_excite(MRS_opt(ii).d_eqm,MRS_opt(ii).H,'x');
    d_A = MRS_opt(ii).d;
    ...
    %MRS_opt(ii).d = outA_temp{Y}; %From here, transfer to the fminsearch loop
 ...
 %%
    ...
    switch (mega_or_hadam)
        ...
        case 'Edited_se_MRSI'
            [outA,outB,~,~]         = sim_megapress_shaped_ultrafast_readout_MRSI(MRS_opt(ii),d_Ay,d_By);%,d_Cy,d_Dy);
        case {'HERMES', 'HERCULES'}
...
function [out1,out2] = sim_megapress_shaped_ultrafast_readout_MRSI(MRS_opt,d1,d2,d3,d4)
%CONTINUE PULSE SEQUENCE************
d1       = sim_apply_pfilter(d1,MRS_opt.H,-1);                                         %Apply pfilter
d2       = sim_apply_pfilter(d2,MRS_opt.H,-1);                                         %Apply pfilter
...
d1   = apply_propagator_edit(d1,MRS_opt.H,MRS_opt.QoutONA);                        %Apply pulse propagator
d2   = apply_propagator_edit(d2,MRS_opt.H,MRS_opt.QoutONB);                        %Apply pulse propagator
