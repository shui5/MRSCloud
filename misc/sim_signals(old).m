...
for ii = 1:length(MRS_opt)
    ...
    switch (mega_or_hadam)
        ...
        case 'HERMES'
            ...
            %dl_split1   = abs(diff([MRS_opt(ii).editOnFreq2,MRS_opt(ii).editOnFreq1]))*MRS_opt(ii).Bfield*MRS_opt(1).gamma/1e6;
        case 'HERCULES'
            editOnFreq1 = MRS_opt(ii).editOnFreq1;
            editOnFreq2 = MRS_opt(ii).editOnFreq2;
            editOnFreq3 = MRS_opt(ii).editOnFreq3;
            editOnFreq4 = MRS_opt(ii).editOnFreq4;
            %dl_split1   = abs(diff([MRS_opt(ii).editOnFreq1,1.9]))*MRS_opt(ii).Bfield*MRS_opt(1).gamma/1e6;
            %dl_split2   = abs(diff([MRS_opt(ii).editOnFreq2,1.9]))*MRS_opt(ii).Bfield*MRS_opt(1).gamma/1e6;
    end
    ...
    switch (mega_or_hadam)
        ...
        otherwise
            ...
            if strcmp(MRS_opt(ii).seq, 'HERCULES')
...
%                 MRS_opt(ii).editRFonA  = rf_freqshift(MRS_opt(ii).editRF11,MRS_opt(ii).editTp,(MRS_opt(ii).centreFreq-editOnFreq1)*MRS_opt(ii).Bfield*MRS_opt(ii).gamma/1e6); %mean([editOnFreq2,editOnFreq4])
...
%                 
%                 MRS_opt(ii).editRF33   = dual_lobe_split_fn(MRS_opt(ii).editRF3,polyval(MRS_opt(ii).split_fit,dl_split2),polyval(MRS_opt(ii).B1power_fit,dl_split2));
%                 MRS_opt(ii).editRFonC  = rf_freqshift(MRS_opt(ii).editRF33,MRS_opt(ii).editTp,(MRS_opt(ii).centreFreq-editOnFreq3)*MRS_opt(ii).Bfield*MRS_opt(ii).gamma/1e6); %mean([editOnFreq3,editOnFreq4])
...
%                 
%                 MRS_opt(ii).editRFonD  = rf_freqshift(MRS_opt(ii).editRF4,MRS_opt(ii).editTp,(MRS_opt(ii).centreFreq-editOnFreq4)*MRS_opt(ii).Bfield*MRS_opt(ii).gamma/1e6); %editOnFreq3
...
%                    MRS_opt(ii).editRF44   = dual_lobe_split_fn(MRS_opt(ii).editRF4,polyval(MRS_opt(ii).split_fit,dl_split2),polyval(MRS_opt(ii).B1power_fit,dl_split2));
            ...
            elseif strcmp(MRS_opt(ii).seq, 'HERMES')
                ...
                MRS_opt(ii).editRFonB  = rf_freqshift(MRS_opt(ii).editRF1,MRS_opt(ii).editTp,(MRS_opt(ii).centreFreq-editOnFreq2)*MRS_opt(ii).Bfield*MRS_opt(ii).gamma/1e6); %editOnFreq2
                MRS_opt(ii).QoutONB    = calc_shapedRF_propagator_edit(MRS_opt(ii).H,MRS_opt(ii).editRFonB,MRS_opt(ii).editTp,MRS_opt(ii).edit_flipAngle,0);
            end
    end
    %     else % scnh for PRESS
